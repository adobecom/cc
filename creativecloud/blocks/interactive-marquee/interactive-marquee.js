...
//     background.appendChild(dataSet[1]);
//     background.appendChild(dataSet[2]);
//     decorateBlockBg(dataSet[0]);
//     decorateBlockBg(dataSet[1]);
...
export async function getExcelData(excelLink) {
  const resp = await fetch(`${excelLink}`);
  const html1 = await resp.json();
  console.log("🚀 ~ file: interactive-marquee.js:90 ~ getExcelData ~ html1:", html1)
  const { data } = html1;
  const arr = [];
  data.forEach((grp) => {
    arr.push(grp);

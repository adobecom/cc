...
function getData(node, node1) {
  ...
  if (text === 'Tryit Image') {
    ...
    const textDesc = node1.innerText.trim();
    desktopTryIttext = textDesc;
    ...
    const imgSrc = getSrc(node1);
    [desktopChnagePhotoImage1, desktopChnagePhotoImage2, desktopChnagePhotoImage3] = imgSrc;
  } else if (text === 'Desktop Change Photo Thumbnails') {
    const imgSrc = getSrc(node1);
    [desktopChangePhotoIcon1, desktopChangePhotoIcon2, desktopChangePhotoIcon3] = imgSrc;
  } else if (text === 'Desktop Change Pattern Images') {
    const imgSrc = getSrc(node1);
    [desktopChangePatternImage1, desktopChangePatternImage2, desktopChangePatternImage3] = imgSrc;
  } else if (text === 'Desktop Change Pattern Thumbnails') {
    const imgSrc = getSrc(node1);
    [desktopChangePatternIcon1, desktopChangePatternIcon2, desktopChangePatternIcon3] = imgSrc;
  } else if (text === 'Desktop Change Color Text') {
    const textDesc = node1.innerText.trim().split(',');
    desktopChangeColorText1 = textDesc[0].trim();
    desktopChangeColorText2 = textDesc[1].trim();
    desktopChangeColorText3 = textDesc[2].trim();
  } else if (text === 'Tablet Marquee Title') {
    const imgSrc = getSrc(node1);
    tabletMarqueeTitle = imgSrc;
  } else if (text === 'Tablet Tryit Text') {
    const textDesc = node1.innerText.trim();
    tabletTryIttext = textDesc;
    ...
    const textDesc = node1.innerText.trim().split(',');
    mobileChangeColorText1 = textDesc[0].trim();
  }
}

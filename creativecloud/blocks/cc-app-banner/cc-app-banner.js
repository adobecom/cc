...
function closeBanner() {
  const appBanner = document.querySelector('.cc-app-banner');
  document.body.classList.remove('cc-has-app-banner');
  appBanner.remove();
}
...
function getDecoratedBannerRight(children) {
  ...
  const appBannerRight = createTag('div', { class: 'cc-app-banner-right' });
  const openButton = createTag('button', { class: 'cc-app-banner-button', role: 'text', 'aria-label': openText }, openText);
...
function getDecoratedAppDetails(children) {
  ...
  const appTitle = createTag('h5', { class: 'cc-app-banner-title' }, children[0].textContent.trim());
  const appDesc = createTag('p', { class: 'cc-app-banner-description' }, children[2].textContent.trim());
  const appStars = createTag('div', { class: 'cc-app-banner-stars', role: 'text', 'aria-label': `Average rating ${rating} stars` });
  ...
  const appBannerDetails = createTag('div', { class: 'cc-app-banner-details' });
  const appReviews = createTag('div', { class: 'cc-app-banner-reviews', 'aria-label': '' }, `(${reviews})`);
...
function getDecoratedBannerLeft(children) {
  const appBannerLeft = createTag('div', { class: 'cc-app-banner-left' });
  const picture = children[1].querySelector('img');
  const iconSrc = picture ? picture.getAttribute('src') : '';
  const icon = createTag('div', { class: 'cc-app-banner-icon' });
...
export default async function init(el) {
  ...
  const appBanner = createTag('div', { class: 'cc-app-banner' });
  // Close banner button on the left, also the icon
  const closeBtn = createTag('div', { class: 'cc-app-banner-close', role: 'text', 'aria-label': 'Close banner' }, '×');
  closeBtn.addEventListener('click', closeBanner);
  const appBannerContent = createTag('div', { class: 'cc-app-banner-content' });
  ...
  document.body.classList.add('cc-has-app-banner');

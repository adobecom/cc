...
function calculateProgress(elementTop, layoutConfig, windowHeight) {
    ...
    if (layoutConfig.isHeroMode) {
        ...
        const travelDistance = Math.max(windowHeight * 0.5, layoutConfig.textHeight);
        endTargetTop = startTargetTop - travelDistance;

        // Safety Clamp for Header
...
export default function init(el) {
    try {
        ...
        const tick = () => {
            const elRect = el.getBoundingClientRect();
            const progress = calculateProgress(elRect.top, layoutState, window.innerHeight);
